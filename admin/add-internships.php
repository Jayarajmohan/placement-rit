...
if (isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO internships (event_date, company_name, details, company_link) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $event_date, $company_name, $description, $company_link);

    if ($stmt->execute()) {
        echo "<script> alert('Internships information inserted into the database successfully.');
