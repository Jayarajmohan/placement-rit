...
if (isset($_POST['submit'])) {
    ...
    $queries = validateInput($_POST['yourqueries'], 'Queries');
    ...
    if (count($errors) === 0) {
        ...
        $sql = "INSERT INTO Queries (first_name, last_name, phone, email, queries) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssss", $first_name, $last_name, $phone, $email, $queries);

        if ($stmt->execute()) {
            $response = "Query submitted successfully!";
        } else {
    ...
    echo "<script>alert('$response'); window.location.href = 'connect_engage_2.php';</script>";
}
